...
def _ollama_embedding_available() -> bool:
    # 기본적으로 로컬/원격 Ollama 모델이 준비되어 있다고 가정합니다.
    return True
...
def search_as_cases(req: SearchRequest, db: Session = Depends(get_db)):
    ...
    from langchain_ollama import OllamaEmbeddings
    emb = OllamaEmbeddings(model="nomic-embed-text") if _ollama_embedding_available() else None
    ...
    if emb is None:
        print("Search RAG: Ollama embedding unavailable, using BM25-only retrieval")
